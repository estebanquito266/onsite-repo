...
<div class="main-card mb-3 card">
    <div class="card-header">
        <span class="col-lg-9">
            <h3 class="mr-3">Listado de Unidades Exteriores Onsite</h3>
        </span>
    ...
    <div class="card-body">
        <div class="collapse border mb-5 pl-3 pr-3 pb-3" id="filtro">
            <form action="{{ url('filtrarUnidadExterior') }}" method="POST">
                {{ csrf_field() }}
        ...
        <table style="width: 100%;" class="table table-hover table-striped table-bordered ">
            ...
            <tbody class="small">
                ...
                <tr>
                    <td>
                        <a href="{{ url('unidadExterior/' . $unidadExterior->id . '/edit'  ) }}">{{$unidadExterior->id}}</a>
                    </td>
                    <td>
                        @if($unidadExterior->sistema_onsite->obra_onsite)
                        [{{ $unidadExterior->sistema_onsite->obra_onsite->id }}] {{ $unidadExterior->sistema_onsite->obra_onsite->nombre }}
                        @else
                        ---
                        @endif
                    </td>
                    <td>
                        [{{$unidadExterior->sistema_onsite->id}}] {{ ($unidadExterior->sistema_onsite?$unidadExterior->sistema_onsite->nombre:'---')}}
                    </td>
                    <td>
                        {{$unidadExterior->serie}}
                    </td>
                    <td>
                        @if($unidadExterior->anclaje_piso == 1) Si @else No @endif
                    </td>
                    <td>
                        @if($unidadExterior->contra_sifon == 1) Si @else No @endif
                    </td>
                    <td>
                        @if($unidadExterior->mm_500_ultima_derivacion_curva == 1) Si @else No @endif
                    </td>
                    <td>
                        {{$unidadExterior->observaciones}}
                    </td>
                    <td>
                        <div class="btn-actions-pane-right actions-icon-btn">
                            <div class="grupo_actions btn-group dropdown"><button type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" class="btn-icon btn-icon-only btn btn-link"><i class="pe-7s-menu btn-icon-wrapper"></i></button>
                                <div tabindex="-1" role="menu" aria-hidden="true" class="grupo_actions_inner dropdown-menu-right rm-pointers dropdown-menu-shadow dropdown-menu-hover-link dropdown-menu" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(28px, 36px, 0px);" x-placement="bottom-end">
                                    <a type="button" tabindex="0" class="dropdown-item etiqueta" data-idunidadexterior="{{$unidadExterior->id}}"><i class="dropdown-icon pe-7s-note"> </i><span>Etiqueta</span></a>
                                </div>
                            </div>
                        </div>
                    </td>
        ...
        <!---- PAGINATE -->
        @if( Request::path() == 'unidadExterior' )
        @include('pagination.default-limit-links', ['paginator' => $unidadesExteriores, 'filters' => ''])
        @else
        @include('pagination.default-limit-links', ['paginator' => $unidadesExteriores, 'filters' => '&texto='. $texto .'&sistema_onsite_id='. $sistema_onsite_id ])
        @endif
        <!----  -->
...
@section('modals')
@include('_onsite.unidadexterioronsite.createEtiquetaModal')
@endsection

@section('scripts')
<script type="text/javascript" src="{!! asset('/assets/js/_onsite/UnidadesExterioresOnsite.js') !!}"></script>
