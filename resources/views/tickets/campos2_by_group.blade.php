            ...
            <div class="col-6">
                <div class="form-group  col-md-12" id="ticket_priority_ticket_id_create">
                    ...
                    <select name="priority_ticket_id" id="priority_ticket_id" class="form-control" required>
                        <option data-days="0" value="">-- Seleccione Prioridad --</option>
                        @if(isset($priorities))
                        @foreach($priorities as $priority)
                        <option data-days="{{$priority->value}}" value="{{$priority->id}}" {{ isset($ticket) && $priority->id == $ticket->priority_ticket_id ? 'selected' : '' }}>{{'['.$priority->id.'] '.$priority->name}}</option>
                        @endforeach
                        @endif
                    </select>
                </div>
            ...
            <div class="col-6">
                <div class="form-group  col-md-12" id="ticket_reason_ticket_id_create">
                    ...
                    <select name="reason_ticket_id" id="motivo_consulta_ticket_id" class="form-control" required>
                        ...
                        <option data-groupid="{{$motivo_consulta->group_ticket_id}}" value="{{$motivo_consulta->id}}" {{ isset($ticket) && $motivo_consulta->id == $ticket->reason_ticket_id ? 'selected' : '' }}>{{'['.$motivo_consulta->id.'] '.$motivo_consulta->name}}</option>
                        @endforeach
                        @endif
                    </select>
                </div>
...
<script type="text/javascript" src="/js/derivaciones.js"></script>
  ...
  document.addEventListener("DOMContentLoaded", function() {
    $(document).ready(function () {
        ...
        function setExpirationDays(days) {
            ...
            $('#expiration_date').val(fechaFormateada);
        }
        

        $('.btn-group .btn.active').each(function() {
            var id_to_show = 'radio_grupo'; // fijo porque solo GRUPO está activo
        ...
        $('#motivo_consulta_ticket_id').on('change', function() {
        ...
        $('#ticket_priority_ticket_id_create').on('change', function() {
