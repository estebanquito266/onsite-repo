...
<div class="card">
        ...
        <div data-parent="#accordion" id="collapseFilter1" aria-labelledby="headingFilter" class="collapse" style="">
            <div class="card-body  pl-5 pb-5">
                <form autocomplete="off" id="frmticketsfilter" method="POST" action="{{(!isset($estado)?'filtrarTickets':($estado=='Cerrado'?'filtrarTicketsCerrados':'filtrarTickets'))}}" accept-charset="UTF-8">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    <input type="hidden" name="estado" value="{{isset($estado)?$estado:''}}" />
                    <input type="hidden" name="filtroagil" value="{{isset($filtroagil)?$filtroagil:''}}" id="filtroagil" />
                    ...
                    <div class="form-row mt-3">
                        ...
                        <div class="col-lg-3">
                            ...
                            <select name="priorities[]" id="type" class="form-control multiselect-dropdown" multiple="multiple">
                                <option value="">-- Seleccione la prioridad --</option>
                                @foreach($priorities as $priority)
                                <option value="{{$priority->id}}" {{ !empty($priorities_selecteds) ? ((in_array($priority->id, $priorities_selecteds)) ? 'selected' : '') : ''}}>{{'['.$priority->id.'] '.$priority->name}}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="form-row mt-3">
                        ...
                        <div class="form-group  col-lg-3 col-md-3">
                            ...
                            <div class="input-group">
                                ...
                                {!! Form::text('fecha_desde',null,['class'=>'form-control', 'data-toggle'=>"datepicker", 'id'=>'fecha_desde', 'autocomplete'=>'off']) !!}
                            </div>
                        </div>
                        <div class="form-group col-lg-3 col-md-3">
                            ...
                            <div class="input-group">
                                ...
                                {!! Form::text('fecha_hasta',null,['class'=>'form-control', 'data-toggle'=>"datepicker", 'id'=>'fecha_hasta', 'autocomplete'=>'off']) !!}
                            </div>
                        </div>
                        ...
                        <div class="form-group  col-lg-3 col-md-3">
                            ...
                            <div class="input-group">
                                ...
                                {!! Form::text('fecha_vto_desde',null,['class'=>'form-control', 'data-toggle'=>"datepicker", 'id'=>'fecha_vto_desde', 'autocomplete'=>'off']) !!}
                            </div>
                        </div>
                        <div class="form-group col-lg-3 col-md-3">
                            ...
                            <div class="input-group">
                                ...
                                {!! Form::text('fecha_vto_hasta',null,['class'=>'form-control', 'data-toggle'=>"datepicker", 'id'=>'fecha_vto_hasta', 'autocomplete'=>'off']) !!}
                            </div>
                        </div>
                    ...
                    <div class="row justify-content-end">
                        <div class="col-md-2">
                            <button id="ticketsfilter" type="submit" class="btn btn-primary btn-block pull-right mt-1" name="boton" value="filtrar">Filtrar</button>
                        </div>
                        ...
                        <div class="col-md-2">
                            <a href="/exportarTickets" class="btn btn-success btn-block pull-right mt-1" >Exportar</a>
                        </div>
        ...
        <div id="filtro_agil" role="group" class="btn-group-sm btn-group btn-group-toggle float-right" data-toggle="buttons">
            <label class="btn btn-shadow btn-primary {{in_array('creadospormi',$checkeds) ? 'active' : ''}} mycheck" data-toggle="tooltip" title="Tickets creados por este usuario.">
                <input type="checkbox" name="filtroagilradio" value="creadospormi" id="creadospormi" autocomplete="off" {{in_array('creadospormi',$checkeds) ? 'checked' : ''}}>
                Míos
            </label>
            <label class="btn btn-shadow btn-primary ml-1 mr-1 {{in_array('asignadosami',$checkeds) ? 'active' : ''}} mycheck" data-toggle="tooltip"  title="Tickets asignados a este usuario.">
                <input type="checkbox" name="filtroagilradio" value="asignadosami" id="asignadosami" autocomplete="off" {{in_array('asignadosami',$checkeds) ? 'checked' : ''}}>
                Asignados
            </label>
            <label class="btn btn-shadow btn-primary {{in_array('asignadosamigrupo',$checkeds) ? 'active' : ''}} mycheck" data-toggle="tooltip"  title="Tickets asignados a este Grupo de usuarios.">
                <input type="checkbox" name="filtroagilradio" value="asignadosamigrupo" id="asignadosamigrupo" autocomplete="off" {{in_array('asignadosamigrupo',$checkeds) ? 'checked' : ''}}>
                Grupo
            </label>
        </div>
        </div>
        ...
        <table style="width: 100%;" id="ticketsdiv" class="table table-hover table-striped table-bordered " data-search="true">
            <thead>
                <tr>
                    <th>#</th>
                    <th>Ingreso</th>
            ...
            <tbody class="small">
                ...
                <tr>
                    <td>
                        {!! link_to_route('ticket.edit', $title = $ticket->id, $parameters = $ticket->id, $attributes = null)!!}
                    </td>
                    ...
                    <td class="text-center">
                        ...
                        @if ($priority_ticket === 'alta')
                            <button data-toggle="tooltip" title="Prioridad alta" class="btn-icon btn-icon-only btn-shadow btn-outline-2x btn btn-outline-danger btn-xs"><i style="margin-top: -2px;" class="lnr-chevron-up-circle btn-icon-wrapper"> </i></button>
                        @elseif ($priority_ticket === 'media')
                            <button data-toggle="tooltip" title="Prioridad media" class="btn-icon btn-icon-only btn-shadow btn-outline-2x btn btn-outline-warning btn-sm"><i style="margin-top: -2px;" class="lnr-circle-minus btn-icon-wrapper"> </i></button>
                        @elseif ($priority_ticket === 'baja')
                            <button data-toggle="tooltip" title="Prioridad baja" class="btn-icon btn-icon-only btn-shadow btn-outline-2x btn btn-outline-success btn-sm"><i style="margin-top: -2px;" class="lnr-chevron-down-circle btn-icon-wrapper"> </i></button>
                    ...
                    <td>{{str_replace("ion","",$ticket->getTypeName())}} {!! !empty($ticket->reparacion_id) ? ': <a style="color:black;" target=" _blank" href="/reparacion/'.$ticket->reparacion_id.'"><b>'.$ticket->reparacion_id.'</b></a>' : (!empty($ticket->derivacion_id) ? ': <a style="color:black;" target=" _blank" href="/derivacion/'.$ticket->derivacion_id.'"><b>'.$ticket->derivacion_id.'</b></a>': '') !!}</td>
                    <td>{{$ticket->cliente ? $ticket->cliente->nombre : ($ticket->cliente_derivacion ? $ticket->cliente_derivacion->nombre : "Sin Cliente")}}</td>
                    ...
                    <td>{{$ticket->expiration_date?date('d/m/Y', strtotime($ticket->expiration_date)):' '}} -
                        ...
                        <span class="badge badge-dot badge-dot-lg {{$ticket->semaforoclass}}" data-toggle="tooltip" title="{{$ticket->semaforo}}">semaforo</span>
                        @endif
                    </td>
                        ...
                        <div class="btn-actions-pane-right actions-icon-btn">
                            <div class="grupo_actions btn-group dropdown">
                                ...
                                <div tabindex="-1" role="menu" aria-hidden="true" class="grupo_actions_inner dropdown-menu-right rm-pointers dropdown-menu-shadow dropdown-menu-hover-link dropdown-menu" style="position: flex; will-change: transform; top: 0px; left: 0px; transform: translate3d(28px, 36px, 0px);" x-placement="bottom-end">
                                    ...
                                    <div class="container">
                                        <div class="row">
                                            <a name='verTicket' data-toggle='modal' href="{{route('ticket.show',$ticket->id)}}" type="button" tabindex="0" class="dropdown-item aprobar_index_btn btn-find-ticket3"><i class="dropdown-icon pe-7s-search"> </i><span>Ver Ticket</span></a>
                                        </div>
                                        <div class="row">
                                            <a name='editarTicket' data-toggle='modal' href="{{route('ticket.edit',$ticket->id)}}" type="button" tabindex="0" class="dropdown-item aprobar_index_btn btn-find-ticket"><i class="dropdown-icon pe-7s-note"> </i><span>Editar Ticket</span></a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="grupo_actions btn-group dropdown">
                                ...
                                <div tabindex="-1" role="menu" aria-hidden="true" class="grupo_actions_inner dropdown-menu-right rm-pointers dropdown-menu-shadow dropdown-menu-hover-link dropdown-menu" style="position: flex; will-change: transform; top: 0px; left: 0px; transform: translate3d(28px, 36px, 0px);" x-placement="bottom-end">
                                    ...
                                    <div class="container">
                                        ...
                                        <div class="row">
                                            <a name='editarTicket' data-toggle='modal' href="{{route('ticket.edit',$ticket->id)}}" type="button" tabindex="0" class="dropdown-item aprobar_index_btn btn-find-ticket"><i class="dropdown-icon pe-7s-ticket"> </i><span>Editar Ticket</span></a>
                                        </div>
                                    </div>
                                </div>
                            </div>
        ...
        @else
        @include('pagination.default-limit-links', ['paginator' => $tickets, 'filters' => '','frmsubmit'=>'frmticketsfilter' ])
        @endif
