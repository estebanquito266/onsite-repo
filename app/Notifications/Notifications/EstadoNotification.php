...
class EstadoNotification extends Notification
{
    ...
    {  
        // $data_mail = $this->mailService->enviarMailEstadoSend($this->array_data);
        $mailable = new EstadoMail($this->mailService,$this->array_data);
        if(isset($this->array_data['reparacion_id']) && $this->array_data['reparacion_id'] >1)
            {
                $email = Reparacion::find($this->array_data['reparacion_id'])->cliente->email;
                $this->array_data['email_to'] =  $email;
            }
        
        Log::info('Estado  Notification: ARRAY: ' . $this->lineaClaseLogs . __LINE__  );
